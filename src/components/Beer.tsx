...
export const Beer: React.FC<BeerType> = ({ image_url, name, description }) => {
    return (
        <Grid sx={{ pl: 0 }} item xs={2} md={3}>
            <Card sx={{ m: 'auto', maxWidth: 340, height: 190, pl: '15px', pt: 5, pb: 5 }}>
                <Box sx={{ display: 'flex' }}>
                    <CardMedia
                        component='img'
                    ...
                    <Box sx={{ display: 'flex', flexDirection: 'column' }}>
                        <CardContent sx={{ height: 200, flex: '1 0 auto', pl: 3, pr: 3, pt: 0, pb: 0, overflow: 'hidden' }}>
                            <Typography component='div' variant='h6' sx={{ textAlign: 'center', mb: 2, lineHeight: 1.2 }}>
                                {name}
                            </Typography>
                            <Typography sx={{ pl: 2, pr: 2, textOverflow: 'ellipsis' }}>
                                {description}
                            </Typography>
                        </CardContent>
                    </Box>
